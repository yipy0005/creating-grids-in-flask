...
for col in warehouse_col:
    ...
    for num in range(0, 11):
        ...
        if num == 0:
            #Linking the start position to starting point of each warehouse column E.g. A0, B0, C0 etc...
        ...
        if num > 0 :
#             #Linking each node in each column to each other E.g. A0 -> A1, A1 -> A2
            warehouse_edges.append([myArray[array_length-2], location])
        if num == 10:     
            warehouse_endpoint_edges.append(location)

### YEW MUN ###
# print("warehouse_edges:\n")
# print(*warehouse_edges,sep='\n')
# print("\nwarehouse_endpoint_edges:\n")
# print(*warehouse_endpoint_edges,sep='\n')
# print("\nmyArray:\n")
# print(*myArray,sep='\n')
...
#Link the edges of warehouse col E.g. A0 -> B0, A10 -> B10, B0 -> C0, B10 -> C10
for idx in range(len(warehouse_endpoint_edges)):
    if idx + 2 < len(warehouse_endpoint_edges):
        warehouse_edges.append([warehouse_endpoint_edges[idx], warehouse_endpoint_edges[idx+2]])
